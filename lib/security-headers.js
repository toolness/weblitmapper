...
function securityHeaders(options) {
  return function(req, res, next) {
    ...
    addContentSecurityPolicy(req, res, options.staticRoot);
    next();
  };
}

function addContentSecurityPolicy(req, res, staticRoot) {
  var policies = {
    'default-src': ["'self'"],
    'script-src': [
      "'self'",
  ...
  if (staticRoot) {
    policies['script-src'].push(staticRoot);
    policies['style-src'].push(staticRoot);
  }
  var directives = [];
