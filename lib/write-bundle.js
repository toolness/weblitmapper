...
var URL = '/js/bundle.js';
var PATH = path.normalize(paths.staticDir + URL);
...
function writeBundle(options) {
  ...
  var bundled = browserifier.bundle({debug: options.debug});
  if (options.debug)
    bundled.on('error', function(err) {
      bundled.end(logError('writing bundle', err));
    });
  if (options.writeToFile)
    bundled.pipe(fs.createWriteStream(PATH));
...
if (!module.parent)
  writeBundle().pipe(process.stdout);
